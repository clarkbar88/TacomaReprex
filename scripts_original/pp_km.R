...
# table by nd status; determine unique x values and number at risk; build KM CDF
  ...
  km.tab <- km.tab %>% dplyr::select(-xx) %>% rename(sum.wt=sw) %>% relocate(km.lev,.before = nd)


# plotting positions for detects
  tmp <- km.tab %>% filter(nd == 0) %>% mutate(km.cdf=km.cdf*(n/(n+1))) %>% dplyr::select(km.lev,nd,km.cdf) %>% rename(x=km.lev,pp=km.cdf)
  f.d <- f %>% filter(nd == 0) %>% left_join(.,tmp,by = c('x','nd'))
  
# plotting positions for NDs
  tmp <- km.tab %>% filter(nd == 1) %>% mutate(km.cdf=km.cdf*(n/(n+1)),km.inc=km.cdf/(nn+1)) %>% dplyr::select(km.lev,nd,km.inc) %>% rename(x=km.lev)
  f.nd <- f %>% filter(nd == 1) %>% arrange(x) %>% left_join(.,tmp,by = c('x','nd')) %>% group_by(x) %>% mutate(pp=cumsum(km.inc)) %>% ungroup() %>% dplyr::select(-km.inc)
